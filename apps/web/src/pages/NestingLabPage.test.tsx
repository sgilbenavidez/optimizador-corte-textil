...
const pieceCodes = ["BACK", "FRONT", "NECKBAND", "SLEEVE", "SLEEVE"];
const marker: MarkerPreview = {
  ...
  marker_length_cm: 83.466, usable_width_cm: 176, physical_width_cm: 180, piece_count: 5, piece_area_total_cm2: 100,
  ...
  placements: pieceCodes.map((pieceCode, index) => ({ piece_instance_id: `M_${pieceCode}_${index}`, pattern_piece_id: `p-${index}`, size_code: "M", piece_code: pieceCode,
    transform: { rotation: 0, mirrored: false }, translation: { x_units: 2000, y_units: 2000, x_cm: 2, y_cm: 2 },
    transformed_polygon: { unit: "geometry_unit", coordinates: [[[2000 + index * 12000, 2000], [12000 + index * 12000, 2000], [12000 + index * 12000, 22000], [2000 + index * 12000, 22000], [2000 + index * 12000, 2000]]] },
    grainline: { unit: "geometry_unit", start: [7000 + index * 12000, 4000], end: [7000 + index * 12000, 20000] }, bbox: { units: [2000 + index * 12000, 2000, 12000 + index * 12000, 22000], cm: [2, 2, 12, 22] }, geometry_hash: `g-${index}`, sequence: index + 1 })),
};
...
test("generates a backend marker and exposes the technical certificate", async () => {
  ...
  expect(screen.getByText("PIEZAS 5 / 5")).toBeInTheDocument();
  const svg = screen.getByRole("img", { name: /Marcador real validado/ });
  expect(svg).toHaveAttribute("viewBox", "0 0 83466 180000");
  expect(svg).toHaveAttribute("preserveAspectRatio", "xMidYMid meet");
  expect(svg.querySelectorAll("[data-marker-polygon]")).toHaveLength(5);
